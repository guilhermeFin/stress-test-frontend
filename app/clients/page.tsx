...
function HealthDot({ score }: { score: number }) {
  const cls = score >= 7 ? 'bg-green-400' : score >= 5 ? 'bg-yellow-400' : score >= 3 ? 'bg-orange-400' : 'bg-red-400'
  return <div className={`w-2 h-2 rounded-full shrink-0 ${cls}`} />
}

function DeltaBadge({ current, previous }: { current: number; previous: number }) {
  const delta = current - previous
  if (Math.abs(delta) < 0.5) return <span className='text-xs text-gray-500'>Unchanged</span>
      ...
      <button
        ...
        <div className='flex-1 min-w-0'>
          <div className='flex items-center gap-2 mb-0.5'>
            ...
            <span className='font-semibold text-white truncate'>{latest.clientName}</span>
          </div>
        ...
        <div className='text-right shrink-0'>
          <div className={`text-lg font-bold tabular-nums ${latest.totalLossPct <= -10 ? 'text-red-400' : 'text-orange-400'}`}>
        ...
        <ChevronRight size={16} className={`text-gray-500 shrink-0 transition-transform
          ${expanded ? 'rotate-90' : ''}`} />
      </button>
          ...
          {previous && (
            <div className='px-5 py-4 border-b border-white/6'>
              ...
              <div className='grid grid-cols-3 gap-3'>
                ...
                <div className='bg-white/3 rounded-xl p-3'>
                  <p className='text-xs text-gray-500 mb-1'>Stress loss</p>
                  <div className='flex items-center gap-2'>
          ...
          <div className='px-5 py-4'>
            ...
            <div className='grid grid-cols-2 md:grid-cols-4 gap-3 mb-4'>
              <div className='bg-white/3 rounded-xl p-3'>
                ...
                <p className='text-sm font-bold text-white'>{fmt(latest.portfolioValue)}</p>
              </div>
              <div className='bg-white/3 rounded-xl p-3'>
                <p className='text-xs text-gray-500 mb-1'>Stressed value</p>
                <p className='text-sm font-bold text-red-400'>{fmt(latest.stressedValue)}</p>
              </div>
              <div className='bg-white/3 rounded-xl p-3'>
                <p className='text-xs text-gray-500 mb-1'>Health score</p>
                <p className={`text-sm font-bold ${healthColor}`}>{latest.healthScore.toFixed(1)}/10</p>
              </div>
              <div className='bg-white/3 rounded-xl p-3'>
                <p className='text-xs text-gray-500 mb-1'>Goals</p>
                <div className='flex items-center gap-1.5'>
                  <GoalIcon size={13} className={goalColor} />
                  <p className={`text-sm font-bold ${goalColor}`}>
            ...
            <div className='space-y-1.5 mb-4'>
              {reviews.map((review, i) => (
                <div key={review.id} className='flex items-center gap-3 py-2 px-3
                  ...
                  <span className='text-xs text-gray-500 flex-1'>{fmtDate(review.date)}</span>
...
export default function ClientsPage() {
  ...
  // Group reviews by client name, sorted newest first within each group
      ...
      <div className='relative max-w-3xl mx-auto px-6 py-12'>
        ...
        <div className='flex items-center justify-between mb-14'>
          ...
          <Link href='/upload'
            className='flex items-center gap-2 px-4 py-2 rounded-xl
              bg-blue-600 hover:bg-blue-500 text-sm text-white font-semibold transition-all'>
            New analysis
        ...
        {!loaded ? (
          ...
          <div className='bg-white/3 border border-white/8 rounded-2xl p-12 text-center'>
            ...
            <h3 className='text-xl font-semibold text-white mb-2'>No reviews saved yet</h3>
